...
class IPCWrap {
    ...
    async onMsg(connection, msg) {
        if (msg.msgType === 'start') {
            ...
            const { obs, reqRep } = m;
            if (reqRep) {
                await this.callReqReplyHandler(connection, reqRep, callNum, requestData);
            }
            else if (obs) {
                this.callObsHandler(connection, obs, callNum, requestData);
            }
        }
    ...
    async callReqReplyHandler(connection, reqRep, callNum, requestData) {
    ...
    callObsHandler(connection, obs, callNum, requestData) {
        const cancel = obs(requestData, {
    ...
    startIPC() {
        return w3n.exposeService(this.srvName, {
            next: connection => this.onConnection(connection, connection.watch({
                next: msg => this.onMsg(connection, msg),
                complete: () => this.onConnectionCompletion(connection),
                error: err => this.onConnectionError(connection, err)
            })),
...
class SingleConnectionIPCWrap extends IPCWrap {
    ...
    async onConnection(connection, disconnect) {
        this.disconnectConnection = disconnect;
    }
    async onConnectionCompletion(connection) {
        w3n.closeSelf();
    }
    async onConnectionError(connection, err) {
        if (w3n.log) {
...
class MultiConnectionIPCWrap extends IPCWrap {
    ...
    stopIPC() {
        for (const disconnect of this.connections.values()) {
            disconnect();
        }
    ...
    async onConnection(connection, disconnect) {
        this.connections.set(connection, disconnect);
    }
    async onConnectionCompletion(connection) {
