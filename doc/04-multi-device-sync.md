...
**Исключение из этого правила — записи о блокировке контакта** (`contact:blocked` /
`contact:unblocked`): они создаются локально и фантома намеренно **не** получают. Причина в том,
что здесь синхронизируется не запись, а её источник: чёрный список ведёт и реплицирует между
устройствами приложение контактов, поэтому вотчер срабатывает на каждом устройстве и каждое пишет
свою строку само. Фантом поверх этого дал бы вторую строку вместо общей. Цена — устройство, не
работавшее между блокировкой и разблокировкой, увидит только итог и не напишет ни одной из строк;
подробнее в [10-contact-blocking.md §5.1](10-contact-blocking.md#51-почему-они-не-синхронизируются-между-устройствами).
