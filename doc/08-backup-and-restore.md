...
Практическое следствие, которое стоит проговорить: если историю очистили **после** снятия архива, то
`merge` её не вернёт — записи архива старше маркера. Вернуть её — работа `replace` (§4.2.1).

### 4.2.1 `replace` и очищенная история

`replace` — единственный режим, который **отменяет очистку истории**: обещание режима в том, что
состояние приводится к архивному, и пользователь, очистивший историю и затем осознанно
восстанавливающий архив, который её предшествует, просит именно этого. Надгробие **отдельного
сообщения** при этом финально в обоих режимах: удаление одного сообщения — решение более позднее и
более узкое, чем архив, и его не отменяет никакой режим.

Возврат обязан быть долговечным, и это не бесплатно. Маркер `historyCleared` остаётся на месте, а
фантом очистки **переигрывается** каждым догоняющим сканом, пока лежит в общем ящике (до 15 дней).
Поэтому запись, поднятая над маркером, штампуется свежим токеном `restoreToken`: без него
воскрешение прожило бы до следующего скана. Токен минтится **один раз** восстанавливающим
устройством и едет в чанках снапшота
([RestoreSnapshotSysMsgData.value.restoreToken](../types/asmail-msgs.types.ts)) — не минтится на
каждом устройстве: два устройства записали бы две разные версии одного аспекта, и следующее
изменение этой записи упорядочивалось бы на них по-разному. Он строго старше `deleted.token`, чтобы
инвариант «удаление побеждает всё, что этот же снапшот восстановил» сохранялся.

Совместимость: чанки старой сборки поля не несут, и приёмная сторона в этом случае ничего не
воскрешает (`replace without a restore token leaves a cleared history alone`) — на время выката
устройства могут разойтись в том, вернулась ли очищенная история; своя очистка и своё восстановление
на каждом из них при этом остаются согласованными.
...
### 7.1 Три диалога создания, и почему пояснения — отдельный шаг

`startBackupWorkflow` ([useBackupRestore.ts](../src-main/common/composables/useBackupRestore.ts))
ведёт цепочку: `backup-passphrase-dialog` («Protect the backup», иконка `shield-alert-outline`) →
`backup-notices-dialog` («Continue» / «Cancel») → `backup-creating-dialog`, внутри которого работа и
файловый диалог платформы.

Средний шаг появился по итогам живого прогона. Четыре пояснения о границе того, что архив вообще
способен вернуть (вложения полученных сообщений остаются в ящике на сервере, едут только файлы
**этого** устройства, файлы больше 20 МиБ приложены ссылкой, чаты с авто-удалением вернут лишь
непросроченное) стояли в диалоге прогресса. Там они видны ровно столько, сколько идёт бэкап, — на
небольшой истории это меньше секунды: достаточно, чтобы заметить, что что-то написали, и
недостаточно, чтобы прочитать. Теперь это остановка, и последняя, с которой ещё можно уйти: после
неё читаются чаты, пакуется архив и платформа спрашивает, куда сохранять. Диалог прогресса —
только стадия и процент.

Тексты пояснений — те же ключи `backup.create.*Notice`; `autoDeleteNotice` до этого был объявлен и
не использован нигде, теперь он четвёртый в списке. Имя фильтра файловых диалогов (`ZIP Archive`)
тоже переехало в i18n (`backup.zipFilterName`): его показывает в своём выпадающем списке платформа,
то есть это подпись для пользователя, как и любая другая.
...
6. **Авто-удаление против архива**: восстановление архива старше окна `autoDeleteMessages` чата
   ...
   пользователь предупреждён перед созданием архива (§7.1).
