...
import { GroupChatStatus, SingleChatStatus } from '../../../../types/chat.types.ts';
import { ChatIdObj } from "../../../../types/asmail-msgs.types.ts";
import { TransformDefinition, andEqualExprFor, forTableInsert, fromQueryResult, queryParamsFrom, setExprFor } from '../../../utils/for-sqlite.ts';

export interface GroupChatDbEntry {
  chatId: string;
  members: string[];
  admins: string[];
  name: string;
...
export interface OTOChatDbEntry {
  peerCAddr: string;
  peerAddr: string;
  name: string;
  createdAt: number;
...
export type ChatDbEntry =
  (GroupChatDbEntry & { isGroupChat: true; })
  | (OTOChatDbEntry & { isGroupChat: false; });

const queryToCreateChatsDbV1 = [
  `--sql
...
const groupChatTabFields: TransformDefinition<GroupChatDbEntry> = {
  chatId: 'as-is',
  name: 'as-is',
  createdAt: 'as-is',
  lastUpdatedAt: 'as-is',
  ...
  admins: {
    toSQLValue: (
      admins: GroupChatDbEntry['admins']
    ): GroupChatDbRecord['admins'] => {
      if (admins.length < 1) {
  ...
  members: {
    toSQLValue: (
      members: GroupChatDbEntry['members']
    ): GroupChatDbRecord['members'] => {
      if (members.length < 1) {
        throw makeDbRecordException({
          invalidChatInsertData: true,
...
const otoChatTabFields: TransformDefinition<OTOChatDbEntry> = {
  peerCAddr: 'as-is',
  peerAddr: 'as-is',
  name: 'as-is',
  status: 'as-is',
...
export function initializeV1chats(db: Database): void {
  db.exec(queryToCreateChatsDbV1);
}
...
export class ChatsDB {
  ...
  async absorbChatRecords(
    groupChats: GroupChatDbEntry[], otoChats: OTOChatDbEntry[]
  ): Promise<void> {
    for (const groupChat of groupChats) {
      ...
      this.sqlite.db.exec(
        `INSERT INTO group_chats (${orderedColumns}) VALUES (${orderedValues})`,
        insertParams
      );
    }
    for (const otoChat of otoChats) {
      ...
      this.sqlite.db.exec(
        `INSERT INTO oto_chats (${orderedColumns}) VALUES (${orderedValues})`,
        insertParams
      );
    }
  ...
  getGroupChat(chatId: string): GroupChatDbEntry|undefined {
    ...
    const [sqlValue] = this.sqlite.db.exec(
      `SELECT * FROM group_chats WHERE ${whereClause}`,
      whereParams
    );
    return (sqlValue ?
      fromQueryResult(sqlValue, groupChatTabFields)[0] : undefined
    );
  }
  ...
  getOTOChat(peerCAddr: string): OTOChatDbEntry|undefined {
    ...
    const [sqlValue] = this.sqlite.db.exec(
      `SELECT * FROM oto_chats WHERE ${whereClause}`,
      whereParams
    );
    return (sqlValue ?
      fromQueryResult(sqlValue, otoChatTabFields)[0] : undefined
    );
  }
  ...
  updateGroupChat(
    ...
    this.sqlite.db.exec(
      `UPDATE group_chats SET ${setExpr} WHERE chatId=$chatId`,
      updateParams
    );
  ...
  addGroupChat(
    params: Omit<GroupChatDbEntry, 'createdAt' | 'lastUpdatedAt'>
  ): GroupChatDbEntry {
  ...
  deleteGroupChat(chatId: string): boolean {
    const { whereClause, whereParams } = groupChatWhereParamsFor(chatId);
    this.sqlite.db.exec(
      `DELETE FROM group_chats WHERE ${whereClause}`,
      whereParams
    );
    return (this.sqlite.db.getRowsModified() > 0);
  }
  ...
  private chatNameExists(name: string): boolean {
    return (
      this.chatNameExistsInOTOs(name) || this.chatNameExistsInGroups(name)
    );
  }
  ...
  deleteOTOChat(peerCAddr: string): boolean {
    const { whereClause, whereParams } = otoChatWhereParamsFor(peerCAddr);
    this.sqlite.db.exec(
      `DELETE FROM oto_chats WHERE ${whereClause}`,
      whereParams
    );
    return (this.sqlite.db.getRowsModified() > 0);
  }
  ...
  updateOTOChat(peerCAddr: string, toUpdate: Partial<OTOChatDbEntry>): boolean {
    ...
    this.sqlite.db.exec(
      `UPDATE oto_chats SET ${setExpr} WHERE peerCAddr=$peerCAddr`,
      updateParams
    );
    return (this.sqlite.db.getRowsModified() > 0);
  }
