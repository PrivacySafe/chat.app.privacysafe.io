...
import { Database, objectFromQueryExecResult, SQLiteOn3NStorage } from '../../../../shared-libs/sqlite-on-3nstorage/index.js';
...
import { TransformDefinition, andEqualExprFor, booleanTransform, forTableInsert, fromQueryResult, optJsonTransform, optStringAsEmptyTransform, queryParamsFrom, setExprFor } from '../../../utils/for-sqlite.ts';
...
export function initializeV1msgs(db: Database): void {
  db.exec(queryToCreateMsgsDbV1);
}
...
export class MsgsDBs {
  ...
  async addMessage(msg: MsgDbEntry): Promise<void> {
    ...
    } = forTableInsert(msg, msgsTabFields);
    this.latestDB.db.exec(
      `INSERT INTO messages(${orderedColumns}) VALUES (${orderedValues})`,
      insertParams
    );
  }
  ...
  async getMessage(id: ChatMessageId): Promise<MsgDbEntry|undefined> {
    ...
    const [ sqlValue ] = this.latestDB.db.exec(
      `SELECT * FROM messages WHERE ${whereMsg}`,
      whereMsgParams
    );
    if (sqlValue) {
  ...
  async msgsInInboxAndOutgoingAttachmentsInOTOChat(
    otoPeerCAddr: string
  ): Promise<RefsToMsgsDataNoInDB> {
    const inboxMsgs = await this.getInboxMsgsOfOTOChat(otoPeerCAddr);
    const outgoingMsgs = await this.getAllOutgoingMsgAttachmentsInOTOChat(
      otoPeerCAddr
    );
    return { inboxMsgs, outgoingMsgs };
  }

  private async getInboxMsgsOfOTOChat(
    otoPeerCAddr: string
  ): Promise<string[]> {
    const whereParams = queryParamsFrom<Pick<
      MsgDbEntry, 'otoPeerCAddr' | 'isIncomingMsg'
    >>(
    ...
    const whereClause = andEqualExprFor(whereParams);
    const [sqlValue] = this.latestDB.db.exec(
      `SELECT incomingMsgId FROM messages
  ...
  private async getAllOutgoingMsgAttachmentsInOTOChat(
    otoPeerCAddr: string
    ...
    const whereClause = andEqualExprFor(whereParams);
    const [sqlValue] = this.latestDB.db.exec(
      `SELECT chatMessageId, attachments FROM messages
  ...
  async deleteMessage(id: ChatMessageId): Promise<boolean> {
    const { whereMsg, whereMsgParams } = msgWhereParamsFor(id);
    this.latestDB.db.exec(
      `DELETE FROM messages WHERE ${whereMsg}`,
      whereMsgParams
    );
    return (this.latestDB.db.getRowsModified() > 0);
  }

  async msgsInInboxAndOutgoingAttachmentsInGroupChat(
    chatId: string
  ): Promise<RefsToMsgsDataNoInDB> {
    const inboxMsgs = await this.getInboxMsgsOfGroupChat(chatId);
    const outgoingMsgs = await this.getAllOutgoingMsgAttachmentsInGroupChat(
      chatId
    );
    return { inboxMsgs, outgoingMsgs };
  }

  private async getInboxMsgsOfGroupChat(
    groupChatId: string
  ): Promise<string[]> {
    const whereParams = queryParamsFrom<Pick<
      MsgDbEntry, 'groupChatId' | 'isIncomingMsg'
    >>(
  ...
  private async getAllOutgoingMsgAttachmentsInGroupChat(
    groupChatId: string
  ): Promise<RefsToMsgsDataNoInDB['outgoingMsgs']> {
    const whereParams = queryParamsFrom<Pick<
      MsgDbEntry, 'groupChatId' | 'isIncomingMsg'
  ...
  async deleteMessagesInOneToOneChat(otoPeerCAddr: string): Promise<boolean> {
    ...
    this.latestDB.db.exec(
      `DELETE FROM messages WHERE ${whereGroup}`,
      whereGroupParams
    );
    return (this.latestDB.db.getRowsModified() > 0);
  }
  ...
  getUnreadMsgsCountIn({ isGroupChat, chatId }: ChatIdObj): number {
    ...
    const whereClause = andEqualExprFor(whereParams);
    const [sqlValue] = this.latestDB.db.exec(
      `SELECT COUNT(*) AS unread FROM messages WHERE ${whereClause}`,
      whereParams
    );
    return (sqlValue ?
      objectFromQueryExecResult<{ unread: number }>(sqlValue)[0].unread : 0
    );
  }

  getLatestIncomingMsgTimestamp(): number|undefined {
    const [sqlValue] = this.latestDB.db.exec(
      `SELECT max(timestamp) as maxTS FROM messages WHERE isIncomingMsg=1`
    );
    const {
      maxTS
    } = objectFromQueryExecResult<{ maxTS: number|null; }>(sqlValue)[0];
    return ((maxTS === null) ? undefined : maxTS);
  }

  async updateMessageRecord(
    id: ChatMessageId, toUpdate: Partial<MsgDbEntry>
  ): Promise<boolean> {
    const { whereMsg, whereMsgParams } = msgWhereParamsFor(id);
    ...
    this.latestDB.db.exec(
      `UPDATE messages SET ${setExpr} WHERE ${whereMsg}`,
      {
        ...updateParams,
        ...whereMsgParams
      }
