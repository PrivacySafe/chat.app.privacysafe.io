...
export default function useChatView() {
  ...
  const { currentChat, currentChatMessages } = storeToRefs(chatStore);
  const { fetchChat, sendMessageInChat } = chatStore;
  const { getChatMessage } = chatsStore;
