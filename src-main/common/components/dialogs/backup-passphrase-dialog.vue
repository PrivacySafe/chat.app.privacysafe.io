...
<script lang="ts" setup>
  ...
  const props = defineProps<{
    /**
     * `create` asks for a new passphrase and can be left empty, which produces an
     * unencrypted archive. `open` asks for the one an archive was made with.
     */
    mode: 'create' | 'open';
  ...
  const isValid = computed(() => {
    ...
    // An empty pair is a legitimate answer: it means "do not encrypt".
