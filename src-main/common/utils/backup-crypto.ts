...
// pulled in for this, and the passphrase never leaves the window: it is turned
// into a key here and never crosses the app's IPC.
//
// This lives in the GUI and NOT in the deno component. On Android a component
// declared with `runtime: "deno"` is executed by androidx.javascriptengine - a
// bare V8 isolate with no Web APIs at all, so `crypto` is simply absent there.
// The GUI is loaded over an https origin on Android and over a scheme registered
// as secure in electron, so `crypto.subtle` is present on both.
//
...
import type { BackupEncryptionParams } from '~/backup.types';
...
/**
 * Whether this runtime can encrypt at all.
 *
 * Checked rather than assumed: a missing `crypto.subtle` must degrade to
 * "encryption unavailable" instead of failing a backup outright.
 */
...
/**
 * Salt and iv come from here rather than from any general-purpose random helper,
 * which is free to fall back on Math.random when crypto is missing. Such a
 * fallback is fine for an id and unacceptable for a salt or a nonce, so a
 * missing entropy source has to be an error.
 */
...
/**
 * base64 of a short byte string.
 *
 * Written out here rather than imported: the only things that go through it are
 * a 16-byte salt and a 12-byte iv, and `btoa` over a string built one char at a
 * time is exact and fast enough for that. The archive itself never passes
 * through base64 - it travels as bytes.
 */
 ...
 *
 * Nothing reaching these calls is ever backed by a shared buffer: the bytes come
 * from cryptoRandomBytes, TextEncoder, base64ToBytes and the zip writer, each of
 * which allocates a plain ArrayBuffer. Copying them just to satisfy the
 ...
 * are written into the container's plaintext metadata file - salt and iv are not
...
export async function decryptPayload(
  ...
  // Read from the archive, so not to be trusted: an unknown algorithm has to say
  // so, rather than reach WebCrypto as a malformed parameter.
